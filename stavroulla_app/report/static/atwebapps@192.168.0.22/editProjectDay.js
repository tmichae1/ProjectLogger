...
function generateEmployeesForm() {
  ...
  for (let i = 0; i < employeeNamesInput.length; i++) {
    employeeNamesInput[i].addEventListener("keydown", function (e) {
      setTimeout(
        handleEmployeeInputChange.bind(null, e.target, employeePriceInput[i]),
        100
      );
    });
  }
...
function generateMachineForm() {
  ...
  for (let i = 0; i < selectBoxes.length; i++) {
    selectBoxes[i].addEventListener("change", function (e) {
      setTimeout(handleMachineInputChange.bind(null, e.target, inputs[i]), 100);
    });
  }
...
// functions fopr inputs
function handleEmployeeInputChange(textInput, numInput) {
  if (textInput.value == "") {
...
function handleMachineInputChange(selectOption, numInput) {
